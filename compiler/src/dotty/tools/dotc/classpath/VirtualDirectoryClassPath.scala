...
import dotty.tools.io.{PlatformURL, PlatformFile}
import FileUtils._
import java.net.URL
...
case class VirtualDirectoryClassPath(dir: VirtualDirectory)
  ...
  protected def getSubDir(packageDirName: String): Option[AbstractFile] =
    Option(
      lookupPath(dir)(
        packageDirName.split(PlatformFile.separator).toIndexedSeq,
        directory = true
      )
    )
  ...
  def asURLs: Seq[PlatformURL] = Seq(PlatformURL(dir.name))
  ...
  def findClassFile(className: String): Option[AbstractFile] = {
    ...
    Option(
      lookupPath(dir)(
        relativePath.split(PlatformFile.separator).toIndexedSeq,
        directory = false
      )
    )
  }
