...
import dotty.tools.io.{PlatformFileInputStream, PlatformProperties, PlatformJarEntry}
...
object Plugin {
  ...
  //   URLClassLoader(urls.toArray, compilerLoader)
  // }
  ...
  def loadAllFrom(
    ...
    dirs: List[Path],
    ignoring: List[String]): List[Try[Plugin]] = List.empty
  // def loadAllFrom(
  //   paths: List[List[Path]],
  //   dirs: List[Path],
