...
import dotty.tools.io.PlatformStandardOpenOption._
...
object File {
  ...
  def apply(path: PlatformPath)(implicit codec: Codec): File = new File(path)
}
...
/** An abstraction for files.  For character data, a Codec
  ...
  */
class File(jpath: PlatformPath)(implicit constructorCodec: Codec)
    extends Path(jpath)
  ...
  def inputStream(): InputStream = PlatformFiles.newInputStream(jpath)
