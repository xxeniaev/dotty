...
object DottyJSPlugin extends AutoPlugin {
  ...
  val writePackageJSON =
    taskKey[Unit]("Write package.json to configure module type for Node.js")
  ...
  override def projectSettings: Seq[Setting[_]] = Def.settings(
    ...
    Compile / jsEnvInput := (Compile / jsEnvInput)
      .dependsOn(writePackageJSON)
      .value,
...
object Build {
  ...
  val dottyGithubRawUserContentUrl =
    "https://raw.githubusercontent.com/lampepfl/dotty"
  ...
  val testCompilation =
    inputKey[Unit]("runs integration test with the supplied filter")

  // Used to compile files similar to ./bin/scalac script
  val scalac = inputKey[Unit](
    "run the compiler using the correct classpath, or the user supplied classpath"
  )

  // Used to run binaries similar to ./bin/scala script
  val scala = inputKey[Unit](
    "run compiled binary using the correct classpath, or the user supplied classpath"
  )
  ...
  val genDocs =
    inputKey[Unit]("run scaladoc to generate static documentation site")

  // Settings used to configure the test language server
  val ideTestsCompilerVersion =
    taskKey[String]("Compiler version to use in IDE tests")
  val ideTestsCompilerArguments =
    taskKey[Seq[String]]("Compiler arguments to use in IDE tests")
  val ideTestsDependencyClasspath =
    taskKey[Seq[File]]("Dependency classpath to use in IDE tests")
  ...
  lazy val commonSettings = publishSettings ++ Seq(
    ...
    Compile / packageBin / packageOptions +=
      Package.ManifestAttributes(
        "Automatic-Module-Name" -> s"${dottyOrganization
          .replaceAll("-", ".")}.${moduleName.value.replaceAll("-", ".")}"
      )
  )
  ...
  lazy val commonDottySettings = commonSettings ++ Seq(
    ...
    classpathOptions ~= (old =>
      old
        .withAutoBoot(
          false
        ) // no library on the compiler bootclasspath - we may need a more recent version
        .withFilterLibrary(
          false
        ) // ...instead, we put it on the compiler classpath
    )
  )
  ...
  private lazy val currentYear: String =
  ...
  def scalacOptionsDocSettings(includeExternalMappings: Boolean = true) = {
    ...
    Seq(
      ...
      "-project-footer",
      s"Copyright (c) 2002-$currentYear, LAMP/EPFL",
      "-author",
      "-groups",
      "-default-template",
      "static-site-main"
  ...
  lazy val commonBootstrappedSettings =
    commonDottySettings ++ NoBloopExport.settings ++ Seq(
      bspEnabled := false,
    ...
    scalacOptions ++= {
      ...
      scalaInstance := {
        val externalLibraryDeps =
          (`scala3-library` / Compile / externalDependencyClasspath).value
            .map(_.data)
            .toSet
        val externalCompilerDeps =
          (`scala3-compiler` / Compile / externalDependencyClasspath).value
            .map(_.data)
            .toSet
        ...
        val compilerJars = Seq(
          ...
          scala3Interfaces,
          scala3Compiler
        ) ++ (externalCompilerDeps -- externalLibraryDeps)
  ...
  lazy val commonBenchmarkSettings = Seq(
    ...
    Jmh / run / mainClass := Some(
      "dotty.tools.benchmarks.Bench"
    ), // custom main for jmh:run
    javaOptions += "-DBENCH_COMPILER_CLASS_PATH=" + Attributed
      .data((`scala3-bootstrapped` / Compile / fullClasspath).value)
      .mkString("", File.pathSeparator, ""),
    javaOptions += "-DBENCH_CLASS_PATH=" + Attributed
      .data((`scala3-library-bootstrapped` / Compile / fullClasspath).value)
      .mkString("", File.pathSeparator, "")
  )
  ...
  val dottyCompilerBootstrappedRef = LocalProject(
  ...
  lazy val `scala3-interfaces` = project
    .in(file("interfaces"))
  ...
  lazy val `scala3-interfaces-js` = project
    .in(file("interfaces-js"))
  ...
  def findArtifact(classpath: Def.Classpath, name: String): File = classpath
    ...
    .getOrElse(
      throw new MessageOnlyException(
        s"Artifact for $name not found in $classpath"
      )
    )
    .data
  ...
  lazy val commonDottyCompilerSettings = Seq(
    ...
    libraryDependencies ++= Seq(
      ...
      ("io.get-coursier" %% "coursier" % "2.0.16" % Test)
        .cross(CrossVersion.for3Use2_13)
    ),
    // For convenience, change the baseDirectory when running the compiler
    Compile / forkOptions := (Compile / forkOptions).value
    ...
    Test / test := {
      ...
      (Test / testOnly)
        .toTask(" -- --exclude-categories=dotty.VulpixMetaTests")
        .value
    },
    (Test / testOptions) += Tests.Argument(
      TestFrameworks.JUnit,
    ...
    javaOptions ++= {
      ...
      Seq(
        ...
        "-Ddotty.tests.classes.compilerInterface=" + findArtifactPath(
          externalDeps,
          "compiler-interface"
        ),
        "-Ddotty.tests.classes.scalaLibrary=" + findArtifactPath(
          externalDeps,
          "scala-library"
        ),
        "-Ddotty.tests.classes.scalaAsm=" + findArtifactPath(
          externalDeps,
          "scala-asm"
        ),
        "-Ddotty.tests.classes.jlineTerminal=" + findArtifactPath(
          externalDeps,
          "jline-terminal"
        ),
        "-Ddotty.tests.classes.jlineReader=" + findArtifactPath(
          externalDeps,
          "jline-reader"
        )
      )
    ...
    testCompilation := Def.inputTaskDyn {
      ...
      if (args.contains("--help")) {
        ...
        val args1 = if (updateCheckfile | fromTasty | rerunFailed)
          args.filter(x =>
            x != "--update-checkfiles" && x != "--from-tasty" && x != "--failed"
          )
        else args
        val test =
          if (fromTasty) "dotty.tools.dotc.FromTastyTests"
          else "dotty.tools.dotc.*CompilationTests dotty.tools.dotc.coverage.*"
        val cmd = s" $test -- --exclude-categories=dotty.SlowTests" +
          (if (updateCheckfile) " -Ddotty.tests.updateCheckfiles=TRUE"
           else "") +
          (if (rerunFailed) " -Ddotty.tests.rerunFailed=TRUE" else "") +
          (if (args1.nonEmpty) " -Ddotty.tests.filter=" + args1.mkString(" ")
           else "")
    ...
    scala := {
      ...
      def run(args: List[String]): Unit = {
        val fullArgs = insertClasspathInArgs(
          args,
          List(".", dottyLib, scalaLib).mkString(File.pathSeparator)
        )
      ...
      if (args.isEmpty) {
        println(
          "Couldn't run `scala` without args. Use `repl` to run the repl or add args to run the dotty application"
        )
      } else if (scalaLib == "") {
        println(
          "Couldn't find scala-library on classpath, please run using script in bin dir instead"
        )
    ...
    scalac := Def.inputTaskDyn {
      ...
      val args = args0.filter(arg =>
        arg != "-repl" && arg != "-decompile" &&
      ...
      val fullArgs = main :: (if (printTasty) args
                              else
                                insertClasspathInArgs(
                                  args,
    ...
    (Compile / sourceGenerators) += Def.task {
      ...
      val scalaJSIRSourcesJar = report
        .select(
      ...
      FileFunction
        .cached(
          cacheDir / s"fetchScalaJSIRSource",
  ...
  def insertClasspathInArgs(args: List[String], cp: String): List[String] = {
    ...
    val classpath =
      fromCp.drop(1).headOption.fold(cp)(_ + File.pathSeparator + cp)
    "-classpath" :: classpath :: beforeCp ::: fromCp.drop(2)
  }

  lazy val nonBootstrapedDottyCompilerSettings =
    commonDottyCompilerSettings ++ Seq(
  ...
  lazy val bootstrapedDottyCompilerSettings =
    commonDottyCompilerSettings ++ Seq(
      javaOptions ++= {
        val jars = packageAll.value
        Seq(
          "-Ddotty.tests.classes.dottyStaging=" + jars("scala3-staging"),
          "-Ddotty.tests.classes.dottyTastyInspector=" + jars(
      ...
      packageAll := {
        (`scala3-compiler` / packageAll).value ++ Seq(
          ...
          "scala3-staging" -> (LocalProject(
            "scala3-staging"
          ) / Compile / packageBin).value.getAbsolutePath,
          "scala3-tasty-inspector" -> (LocalProject(
            "scala3-tasty-inspector"
          ) / Compile / packageBin).value.getAbsolutePath,
          "tasty-core" -> (LocalProject(
  ...
  lazy val `scala3-compiler` =
    project.in(file("compiler")).asDottyCompiler(NonBootstrapped)
  lazy val `scala3-compiler-js` = project
    .in(file("compiler"))
    .enablePlugins(ScalaJSPlugin)
  ...
  lazy val Scala3CompilerCoursierTest =
    config("scala3CompilerCoursierTest") extend Test
  lazy val `scala3-compiler-bootstrapped` = project
    .in(file("compiler"))
    ...
    .settings(
      ...
      Scala3CompilerCoursierTest / envVars := Map(
        "DOTTY_BOOTSTRAPPED_VERSION" -> dottyVersion
      ),
  ...
  lazy val `scala3-library` =
    project.in(file("library")).asDottyLibrary(NonBootstrapped)
  lazy val `scala3-library-js` = project
    .in(file("library"))
    .enablePlugins(ScalaJSPlugin)
    .settings(commonNonBootstrappedSettings)
    .settings(
  ...
  lazy val `scala3-library-bootstrapped`: Project =
    project.in(file("library")).asDottyLibrary(Bootstrapped)
  ...
  /** The dotty standard library compiled with the Scala.js back-end, to produce
    ...
    */
  lazy val `scala3-library-bootstrappedJS`: Project = project
    .in(file("library-js"))
    ...
    .settings(
      ...
      scalacOptions ++= {
        if (isRelease) {
          ...
          Seq(
            s"-scalajs-mapSourceURI:$baseURI->$dottyGithubRawUserContentUrl/$dottyVersion/"
          )
        } else {
          Nil
        }
  ...
  lazy val `tasty-core-js` = project
    .in(file("tasty"))
    .enablePlugins(ScalaJSPlugin)
  ...
  lazy val `tasty-core-bootstrapped`: Project =
    project.in(file("tasty")).asTastyCore(Bootstrapped)
  lazy val `tasty-core-scala2`: Project =
    project.in(file("tasty")).asTastyCoreScala2
  ...
  lazy val `scala3-staging` = project
    .in(file("staging"))
    ...
    dependsOn(
      dottyCompiler(Bootstrapped) % "provided; compile->runtime; test->test"
    )
    .settings(
      javaOptions := (`scala3-compiler-bootstrapped` / javaOptions).value
    )

  lazy val `scala3-tasty-inspector` = project
    .in(file("tasty-inspector"))
    .withCommonSettings(Bootstrapped)
    .
    // We want the compiler to be present in the compiler classpath when compiling this project but not
  ...
  lazy val `stdlib-bootstrapped` = project
    ...
    .settings(
      ...
      transitiveClassifiers := Seq("sources"),
      libraryDependencies +=
        ("org.scala-lang" % "scala-library" % stdlibVersion(
          Bootstrapped
        ) % "sourcedeps"),
      (Compile / sourceGenerators) += Def.task {
        val s = streams.value
        ...
        val report = updateClassifiers.value
        val scalaLibrarySourcesJar = report
          .select(
            configuration = configurationFilter("sourcedeps"),
        ...
        FileFunction
          .cached(
            cacheDir / s"fetchScalaLibrarySrc",
            FilesInfo.lastModified,
            FilesInfo.exists
          ) { dependencies =>
      ...
      (Compile / sourceGenerators) += Def.task {
        ...
        val dottyLibSourceDirs =
          (`scala3-library-bootstrapped` / Compile / unmanagedSourceDirectories).value
        def dottyLibSources =
          dottyLibSourceDirs.foldLeft(PathFinder.empty) { (pf, dir) =>
            if (!dir.exists) pf
            else pf +++ (dir ** "*.scala") +++ (dir ** "*.java")
          }
        ...
        val cachedFun = FileFunction.cached(
          ...
          dottyLibSourceDirs.foreach { dir =>
            if (dir.exists) {
              s.log
                .info(s"Copying scala3-library sources from $dir to $trgDir...")
      ...
      (Test / managedClasspath) ~= {
        _.filterNot(file =>
          file.data.getName == s"scala-library-${stdlibVersion(Bootstrapped)}.jar"
        )
      }
    )
  ...
  /** Test the tasty generated by `stdlib-bootstrapped`
    ...
    */
  lazy val `stdlib-bootstrapped-tasty-tests` = project
    .in(file("stdlib-bootstrapped-tasty-tests"))
    .withCommonSettings(Bootstrapped)
    .dependsOn(`scala3-tasty-inspector` % "test->test")
    .settings(commonBootstrappedSettings)
    .settings(
  ...
  lazy val `scala3-sbt-bridge` = project
    ...
    .settings(
      ...
      Test / test := (LocalProject(
        "scala3-sbt-bridge-tests"
      ) / Test / test).value,
  ...
  lazy val `scala3-sbt-bridge-tests` = project
    .in(file("sbt-bridge/test"))
  ...
  lazy val `scala3-language-server` = project
    ...
    .settings(
      ...
      ideTestsDependencyClasspath := {
        ...
        val scalaLib =
          (`scala3-library-bootstrapped` / Compile / dependencyClasspath).value
            .map(_.data)
  ...
  /** A sandbox to play with the Scala.js back-end of dotty.
    ...
    */
  lazy val sjsSandbox = project
    .in(file("sandbox/scalajs"))
    .enablePlugins(DottyJSPlugin)
  ...
  /** Scala.js test suite.
    ...
    */
  lazy val sjsJUnitTests = project
    .in(file("tests/sjs-junit"))
    .enablePlugins(DottyJSPlugin)
    .dependsOn(`scala3-library-bootstrappedJS`)
    .settings(
      ...
      fetchScalaJSSource := {
        ...
        git
          .getRepository()
          .getConfig()
          .setEnum("core", null, "autocrlf", CoreConfig.AutoCRLF.FALSE)
      ...
      scalaJSLinkerConfig ~= {
        _.withSemantics(build.TestSuiteLinkerOptions.semantics _)
      },
      ...
      Test / jsEnvInput := {
        val resourceDir =
          fetchScalaJSSource.value / "test-suite/js/src/test/resources"
        ...
        (
          (dir / "shared/src/test/scala" ** (("*.scala": FileFilter)
            ...
            ++ conditionally(
              ...
              "js/src/test/require-multi-modules"
            )
            ++ conditionally(
              moduleKind == ModuleKind.ESModule,
              "js/src/test/require-dynamic-import"
            )
            ++ conditionally(
              moduleKind == ModuleKind.ESModule,
              "js/src/test/require-esmodule"
            )
        )
      },
      ...
      Test / testOptions := Seq(
        Tests
          .Filter(_ != "org.scalajs.testsuite.javalib.lang.IterableDefaultTest")
      ),
      Test / managedResources ++= {
        ...
        val moduleSpecific = scalaJSLinkerConfig.value.moduleKind match {
          ...
          case ModuleKind.CommonJSModule =>
            (testDir / "resources-commonjs" ** "*.js").get
          case ModuleKind.ESModule =>
            (testDir / "resources-esmodule" ** "*.js").get
        }
  ...
  lazy val sjsCompilerTests = project
    .in(file("sjs-compiler-tests"))
    ...
    .settings(
      ...
      javaOptions ++= {
        val externalJSDeps =
          (`scala3-library-bootstrappedJS` / Compile / externalDependencyClasspath).value
        val dottyLibraryJSJar =
          (`scala3-library-bootstrappedJS` / Compile / packageBin).value.getAbsolutePath

        Seq(
          "-Ddotty.tests.classes.dottyLibraryJS=" + dottyLibraryJSJar,
          "-Ddotty.tests.classes.scalaJSJavalib=" + findArtifactPath(
            externalJSDeps,
            "scalajs-javalib"
          ),
          "-Ddotty.tests.classes.scalaJSLibrary=" + findArtifactPath(
            externalJSDeps,
  ...
  lazy val `scala3-bench` =
    project.in(file("bench")).asDottyBench(NonBootstrapped)
  lazy val `scala3-bench-bootstrapped` =
    project.in(file("bench")).asDottyBench(Bootstrapped)
  lazy val `scala3-bench-run` =
    project.in(file("bench-run")).asDottyBench(Bootstrapped)

  lazy val `scala3-bench-micro` = project
    .in(file("bench-micro"))
    .asDottyBench(Bootstrapped)
    .settings(Jmh / run / mainClass := Some("org.openjdk.jmh.Main"))

  val testcasesOutputDir =
    taskKey[Seq[String]]("Root directory where tests classses are generated")
  val testcasesSourceRoot =
    taskKey[String]("Root directory where tests sources are generated")
  val testDocumentationRoot =
    taskKey[String]("Root directory where tests documentation are stored")
  val generateSelfDocumentation =
    taskKey[Unit]("Generate example documentation")
  // Note: the two tasks below should be one, but a bug in Tasty prevents that
  val generateScalaDocumentation =
    inputKey[Unit]("Generate documentation for dotty lib")
  val generateStableScala3Documentation =
    inputKey[Unit]("Generate documentation for stable dotty lib")
  val generateTestcasesDocumentation = taskKey[Unit](
    "Generate documentation for testcases, usefull for debugging tests"
  )

  val generateReferenceDocumentation =
    inputKey[Unit]("Generate language reference documentation for Scala 3")

  lazy val `scaladoc-testcases` = project
    .in(file("scaladoc-testcases"))
  ...
  /** Collection of projects building targets for scaladoc, these are:
    ...
    */
  lazy val `scaladoc-js-common` = project
    .in(file("scaladoc-js/common"))
    .enablePlugins(DottyJSPlugin)
    .dependsOn(`scala3-library-bootstrappedJS`)
    .settings(
  ...
  lazy val `scaladoc-js-main` = project
    .in(file("scaladoc-js/main"))
    .enablePlugins(DottyJSPlugin)
  ...
  lazy val `scaladoc-js-contributors` = project
    .in(file("scaladoc-js/contributors"))
    .enablePlugins(DottyJSPlugin)
    .dependsOn(`scaladoc-js-common`)
    .settings(
      Test / fork := false,
  ...
  def generateDocumentation(
      configTask: Def.Initialize[Task[GenerationConfig]]
  ) =
    Def.taskDyn {
  ...
  def bundleCSS = Def.task {
    ...
    def createBundle(dir: File): Seq[File] = {
      ...
      val targetDir = (Compile / resourceManaged).value.toPath
        .resolve(unmanagedResources.toPath.relativize(dir.toPath))
        .toFile
  ...
  val SourceLinksIntegrationTest =
    config("sourceLinksIntegrationTest") extend Test
  ...
  lazy val scaladoc = project
    ...
    .settings(
      ...
      SourceLinksIntegrationTest / test := ((SourceLinksIntegrationTest / test) dependsOn generateScalaDocumentation
        .toTask("")).value
    )
    .settings(
      ...
      BuildInfoPlugin.buildInfoDefaultSettings,
      Test / test := (Test / test)
        .dependsOn(`scaladoc-testcases` / Compile / compile)
        .value,
      Test / testcasesOutputDir := (`scaladoc-testcases` / Compile / products).value
        .map(_.getAbsolutePath),
      ...
      generateScalaDocumentation := Def.inputTaskDyn {
        ...
        val justAPIArg: Option[String] =
          extraArgs.drop(1).find(_ == "--justAPI")
      ...
      generateReferenceDocumentation := Def.inputTaskDyn {
        val shouldRegenerateExpectedLinks =
          (Space ~> literal("--no-regenerate-expected-links")).?.parsed.isEmpty
        ...
        for (
          ...
          val path =
            temp / "docs" / "_docs" / "reference" / name / s"${name}.md"
          val contentLines =
            IO.read(path).linesIterator.to[collection.mutable.ArrayBuffer]
          contentLines
            .insert(1, s"redirectFrom: /${name}.html") // Add redirection
        ...
        val expectedLinksRegeneration = Def.task {
          if (shouldRegenerateExpectedLinks) {
            val script =
              (file("project") / "scripts" / "regenerateExpectedLinks").toString
            val outputDir =
              languageReferenceConfig.value.get[OutputDir].get.value
            val expectedLinksFile = (file(
              "project"
            ) / "scripts" / "expected-links" / "reference-expected-links.txt").toString
            import _root_.scala.sys.process._
            s"$script $outputDir $expectedLinksFile".!
          }
        }

        expectedLinksRegeneration.dependsOn(
  ...
  lazy val `sbt-test` = project
    ...
    .settings(
      ...
      scriptedLaunchOpts ++= {
        val repositoryPath =
          (io.Path.userHome / ".sbt" / "repositories").absolutePath
        s"-Dsbt.repository.config=$repositoryPath" ::
          ivyPaths.value.ivyHome
            .map("-Dsbt.ivy.home=" + _.getAbsolutePath)
            .toList
      },
  ...
  lazy val `sbt-community-build` = project
    .in(file("sbt-community-build"))
  ...
  val prepareCommunityBuild = taskKey[Unit](
    "Publish local the compiler and the sbt plugin. Also store the versions of the published local artefacts in two files, community-build/{scala3-bootstrapped.version,sbt-injected-plugins}."
  )

  lazy val `community-build` = project
    .in(file("community-build"))
    ...
    .settings(
      prepareCommunityBuild := {
        ...
        IO.write(
          baseDirectory.value / "scala3-bootstrapped.version",
          dottyVersion
        )
        IO.delete(
          baseDirectory.value / "dotty-community-build-deps"
        ) // delete any stale deps file
      },
      (Test / testOptions) += Tests.Argument(
        TestFrameworks.JUnit,
      ...
      Compile / run := (Compile / run)
        .dependsOn(prepareCommunityBuild)
        .evaluated,
  ...
  implicit class ProjectDefinitions(val project: Project) extends AnyVal {
    ...
    def asDottyCompiler(implicit mode: Mode): Project =
    ...
    def withCommonSettings(implicit mode: Mode): Project =
...
object ScaladocConfigs {
  ...
  def javaExternalMapping =
    ".*java/.*::javadoc::https://docs.oracle.com/javase/8/docs/api/"
  def scalaSrcLink(v: String, s: String) =
    s"${s}github://scala/scala/v$v#src/library"
  def dottySrcLink(
      v: String,
  ...
  def defaultSourceLinks(
      version: String = dottyNonBootstrappedVersion,
      refVersion: String = dottyVersion
  ) = Def.task {
    def stdLibVersion = stdlibVersion(NonBootstrapped)
    def srcManaged(v: String, s: String) =
      s"out/bootstrap/stdlib-bootstrapped/scala-$v/src_managed/main/$s-library-src"
    SourceLinks(
      List(
        scalaSrcLink(stdLibVersion, srcManaged(version, "scala") + "="),
        dottySrcLink(
          refVersion,
          srcManaged(version, "dotty") + "=",
          "#library/src"
        ),
  ...
  lazy val DefaultGenerationConfig = Def.task {
    ...
    def skipByRegex =
      SkipByRegex(List(".+\\.internal($|\\..+)", ".+\\.impl($|\\..+)"))
    def skipById = SkipById(
      List(
    ...
    def projectFooter =
      ProjectFooter(s"Copyright (c) 2002-$currentYear, LAMP/EPFL")
  ...
  lazy val Scala3 = Def.task {
    ...
    val tastyCoreSources = projectRoot.relativize(
      (`tasty-core-bootstrapped` / Compile / scalaSource).value
        .toPath()
        .normalize()
    )

    val dottyLibRoot =
      projectRoot.relativize(dottyManagesSources.toPath.normalize())
    DefaultGenerationConfig.value
      .add(ProjectName("Scala 3"))
      .add(
        OutputDir(
          file("scaladoc/output/scala3").getAbsoluteFile.getAbsolutePath
        )
      )
      .add(Revision("main"))
  ...
  def stableScala3(version: String) = Def.task {
    Scala3.value
      .add(
        defaultSourceLinks(
          version + "-bin-SNAPSHOT-nonbootstrapped",
          version
        ).value
      )
      ...
      .add(
        DocRootContent(
          s"out/bootstrap/stdlib-bootstrapped/scala-$version-bin-SNAPSHOT-nonbootstrapped/src_managed/main/scala-library-src/rootdoc.txt"
        )
      )
      .withTargets(
        Seq(
